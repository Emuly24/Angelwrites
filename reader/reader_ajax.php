<?php
// ============================================================
//  READER_AJAX.PHP – All AJAX endpoints for the reader
//  Phase 2 – Progress, Bookmarks, Highlights, Annotations,
//  Sessions, Streaks, Goals, Questions, Notifications
// ============================================================
...
// Only logged‑in users can use these endpoints
if (!isLoggedIn()) {
...
if ($action === 'save_position') {
    ...
    // Update reading session duration (if active)
    updateReadingSessionDuration($user_id, $book_id);

    // Check for milestones (50%, 100%) – triggers admin notification
...
if ($action === 'add_reader_note') {
    ...
    $stmt = $db->prepare("
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$group_id, $_SESSION['user_id'], $book_id, $chapter, $text, $is_private]);

    echo json_encode(['success' => true]);
    exit;
}
// ===== GET NOTES (with reactions) =====
if ($action === 'get_notes') {
    $group_id = (int)$_GET['group_id'];
    $book_id = (int)$_GET['book_id'];
    $chapter = (int)$_GET['chapter'];
...
if ($action === 'add_reader_reaction') {
    ...
    $stmt->execute([$note_id, $_SESSION['user_id'], $reaction_type]);
    if (!$stmt->fetch()) {
...
function updateReadingSessionDuration($user_id, $book_id) {
    global $db;
    // This is called periodically – updates the duration of the current session
