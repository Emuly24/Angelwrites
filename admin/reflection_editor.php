...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['audio_recording']) && $_FILES['audio_recording']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['audio_recording']['tmp_name'], $upload_dir . $rec_filename)) {
            // Store audio path – you may need to add a field to blog_posts or handle separately
            // For now, we'll just store it in a custom field or session
            $_SESSION['audio_path'] = 'assets/uploads/audio/' . $rec_filename;
        } else {
    ...
    if (isset($_FILES['video_recording']) && $_FILES['video_recording']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['video_recording']['tmp_name'], $upload_dir . $video_filename)) {
            $_SESSION['video_path'] = 'assets/uploads/videos/' . $video_filename;
        } else {
    ...
    if (empty($title) || empty($content)) {
        ...
        if ($id > 0) {
            ...
            $success = 'Reflection updated successfully!';
        } else {
            ...
            $id = $db->lastInsertId();
            $success = 'Reflection created successfully!';

            // Admin notification via Zoho SMTP
            $admin_email = 'angelwrites@zohomail.com';
            $subject = 'New Christian Reflection: ' . $title;
            $body = "A new reflection has been added.\n\nTitle: $title\nCategory: $category\n\nView reflection: " . SITE_URL . "/blog_post.php?slug=" . $slug;
            sendEmail($admin_email, $subject, $body, 'angelwrites@zohomail.com', SITE_NAME . ' Admin');
        }

        if ($action === 'save_and_continue') {
            header('Location: ' . SITE_URL . '/admin/reflection_editor.php?id=' . $id);
            exit;
        } else {
...
$pageTitle = $id > 0 ? 'Edit Reflection' : 'Add New Reflection';
?>
...
<div class="admin-editor">
    <div class="container">
        <div class="admin-header">
            <h1><?php echo $id > 0 ? 'Edit Reflection' : 'Add New Reflection'; ?></h1>
            <div class="admin-actions">
                <a href="<?php echo SITE_URL; ?>/admin/manage_reflections.php" class="btn btn-outline">
